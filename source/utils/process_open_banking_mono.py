...
import urllib3
import urllib.parse
from utils.standard_func import add_employer_labelisation, del_employer_labelisation
...
from utils.utils import OPEN_BANKING_MONO_COLUMNS_MAPPING, read_json

mono_sec_key = "test_sk_kp8zb8qh7w5jqr0un5zh"
acc_url = "https://api.withmono.com"

COLUMNS_MAPPING = {
            "Date" : ["Tran. date", "Date", "DATE","date"],
            "TransDet" : ["Transaction details", "TRANSACTION","narration"],
            "TypeTrans" : ["type"],
...
def get_detail_account(id_account):
    try:
        headers = {
            'accept': 'application/json',
            'Content-Type': 'application/json',
        ...
        # response_auth = get_auth()
        # id_account=response_auth['id']

        url = ""+str(acc_url)+"/accounts/"+str(id_account)+""
...
def get_income(id_account):
    try:
        ...
        payload = {}
        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/income"
...
def get_identity(id_account):
    try:
        ...
        payload = {}
        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/identity"
...
def get_statement(id_account,month ="last6months" ,output="json"):
    try:
        ...
        # Définir les paramètres de la période
        params = {'period': month}

        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/statement"
...
def get_transaction(id_account):
    try:
        headers = {
            'accept': 'application/json',
            'Content-Type': 'application/json',
        ...
        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/transactions"

        # params = {"start": start, "end":end, "narration": narration,"paginate": paginate}
        # url_with_params = f"{url}?{urllib.parse.urlencode(params)}"
...
def get_earning(id_account):
    try:
        ...
        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/earnings"
...
def get_credits(id_account):
    try:
        ...
        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/credits"
...
def get_debits(id_account):
    try:
        ...
        url = ""+str(acc_url)+"/accounts/"+str(id_account)+"/debits"
...
def open_banking_mono_classify_transaction(row, categories):
...
def open_banking_mono_transaction_labelisation(data, new_key_salary="", categories=LABELISATION):
    try:
        ...
        data['LABEL'] = data.apply(lambda row: open_banking_mono_classify_transaction(row, categories), axis=1)
...
def open_banking_mono_from_to(data, categories=LABELISATION, KEYWORDS=KEYWORDS):
    try:
...
def open_banking_mono_calcul_score(data):
    tps1 = time.time()
...
#     id_account = get_auth("code_7o9PAzh5h1vIN19rBhHjKgSn")
...
#     print(data)
#     #     get_auth("code_7o9PAzh5h1vIN19rBhHjKgSn")
#     #     # get_detail_account()
#     #     # get_income()
#     #     # get_identity()
#     #     # get_statement("6606e1a8b19da202894fbf81")
#     #     get_transaction("6606e1a8b19da202894fbf81")
#     #     # get_earning()
#     #     # get_credits()
#     #     # get_debits()
