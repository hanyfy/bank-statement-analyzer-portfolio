...
def eco_bank_pdf_reader(pdf_path, password):
...
def eco_bank_classify_transaction(row, categories):
...
def eco_bank_transaction_labelisation(data, new_key_salary="", categories=LABELISATION):
    try:
        ...
        data['LABEL'] = data.apply(lambda row: eco_bank_classify_transaction(row, categories), axis=1)
        categories = del_employer_labelisation(categories)
        return data
    except Exception as e:
        print(f"[ERROR][utils.process_eco_bank.labelisation] {e}")
        data['LABEL'] = ""
        return data
...
def eco_bank_from_to(data, categories=LABELISATION, KEYWORDS=KEYWORDS):
    try:
        ...
        print(f"[ERROR][utils.process_eco_bank.eco_bank_from_to] {e}")
        data['FROM'] = ""
        data['TO'] = ""
        return data


def eco_bank_parse_date(df, colomn_date, list_formats=[]):
    
    try:
        # Création de la nouvelle colonne 'Date_Parse'
        df['Datetime'] = pd.to_datetime(df[colomn_date],  infer_datetime_format = True, errors='coerce')
...
def eco_bank_calcul_score(data):
    tps1 = time.time()
...
if __name__ == '__main__':
    ...
    data = eco_bank_pdf_reader(pdf_path, password)    # pdf reader
    print(data.columns)
