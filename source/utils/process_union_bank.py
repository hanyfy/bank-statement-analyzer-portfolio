...
def union_parse_date(df, colomn_date, list_formats=[]):
    
    try:
        # Création de la nouvelle colonne 'Date_Parse'
        if "%d/%m/%Y" in list_formats:
            df['Datetime'] = pd.to_datetime(df[colomn_date], errors='coerce', format="%d/%m/%Y")
        else:
...
def union_bank_pdf_reader(pdf_path, password):
...
def union_bank_classify_transaction(row, categories):
...
def union_bank_transaction_labelisation(data, new_key_salary="", categories=LABELISATION):
    try:
        ...
        data['LABEL'] = data.apply(lambda row: union_bank_classify_transaction(row, categories), axis=1)
        categories = del_employer_labelisation(categories)
        return data
    except Exception as e:
        print(f"[ERROR][utils.process_union_bank.labelisation] {e}")
        data['LABEL'] = ""
        return data
...
def union_bank_from_to(data, categories=LABELISATION, KEYWORDS=KEYWORDS):
    try:
        ...
        print(f"[ERROR][utils.process_union_bank.union_bank_from_to] {e}")
        data['FROM'] = ""
        data['TO'] = ""
        return data


def union_bank_parse_date(df, colomn_date, list_formats=[]):
...
def union_bank_calcul_score(data):
    tps1 = time.time()
...
if __name__ == '__main__':
    ...
    data = union_bank_pdf_reader(pdf_path, password)    # pdf reader
    print(data.columns)
    ...
    data = union_bank_from_to(data, categories=LABELISATION)

    print(data[["FROM", "TO"]])
    union_bank_calcul_score(data)
