...
def wema_bank_pdf_reader(pdf_path, password):
...
def wema_bank_classify_transaction(row, categories):
...
def wema_bank_transaction_labelisation(data, new_key_salary="", categories=LABELISATION):
    try:
        ...
        data['LABEL'] = data.apply(lambda row: wema_bank_classify_transaction(row, categories), axis=1)
        categories = del_employer_labelisation(categories)
        return data
    except Exception as e:
        print(f"[ERROR][utils.process_wema_bank.labelisation] {e}")
        data['LABEL'] = ""
        return data
...
def wema_bank_from_to(data, categories=LABELISATION, KEYWORDS=KEYWORDS):
    try:
        ...
        print(f"[ERROR][utils.process_wema_bank.wema_bank_from_to] {e}")
        data['FROM'] = ""
        data['TO'] = ""
        return data


def wema_bank_parse_date(df, colomn_date, list_formats=[]):
...
def wema_bank_calcul_score(data):
    tps1 = time.time()
...
if __name__ == '__main__':
    ...
    data = wema_bank_pdf_reader(pdf_path, password)    # pdf reader
    print(data.columns)
    ...
    data = wema_bank_from_to(data, categories=LABELISATION)

    print(data[["FROM", "TO"]])
    wema_bank_calcul_score(data)
