...
LABELISATION = {
            ...
            "airtime-spend" : {"keywords":["airtime", "vtup"],"montant" : "negative"},
            "bankcharges-spend" : {"keywords":["charge", "levy", "debitsessioncharge", "vat", "insurance", "nipfee", "nipvat"],"montant" : "negative"},
            "bundle-spend" : {"keywords":["bundle", "ussd data"],"montant" : "negative"},
...
def sterling_bank_pdf_reader(pdf_path, password):
...
def sterling_bank_classify_transaction(row, categories):
...
def sterling_bank_transaction_labelisation(data, new_key_salary="", categories=LABELISATION):
    try:
        ...
        data['LABEL'] = data.apply(lambda row: sterling_bank_classify_transaction(row, categories), axis=1)
        categories = del_employer_labelisation(categories)
        return data
    except Exception as e:
        print(f"[ERROR][utils.process_sterling_bank.labelisation] {e}")
        data['LABEL'] = ""
        return data
...
def extract_from_and_to(text, all_keywords):
    ...
    elif from_index != -1 and to_index != -1 and from_index > to_index:
        from_text = ' '.join(words[from_index + 1:])
    else:
        from_text = ""
...
def sterling_bank_from_to(data, categories=LABELISATION, KEYWORDS=KEYWORDS):
    try:
        ...
        print(f"[ERROR][utils.process_sterling_bank.sterling_bank_from_to] {e}")
        data['FROM'] = ""
        data['TO'] = ""
        return data


def sterling_parse_date(df, colomn_date, list_formats=[]):
    try:
        # Création de la nouvelle colonne 'Date_Parse'
        df['Datetime'] = pd.to_datetime(df[colomn_date], format='%m/%d/%Y', errors='coerce')
...
def sterling_bank_calcul_score(data):
    tps1 = time.time()
...
if __name__ == '__main__':
    ...
    data = sterling_bank_pdf_reader(pdf_path, password)    # pdf reader
    print(data.columns)
    ...
    data = sterling_bank_from_to(data, categories=LABELISATION)

    print(data[["FROM", "TO"]])
    sterling_bank_calcul_score(data)
