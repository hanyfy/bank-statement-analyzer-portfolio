...
KEYWORDS = ['debitsession', 'hbr', 'ustmtn', 'psd', 'fgn', 
            'nxg', 'fee', 'alert', 'sms', 'sac', 'cwu', 'wdl', "hyd", 
            "airtime", "topup", "qtm","ustmtn", "ust", "mtn",
            "nnl", "fip", "gtb", ":", "charges", "withdrawal",
            "withdrawal-fbn"]


def fcmb_parse_date(df, colomn_date, list_formats=[]):
    
    try:
        # Création de la nouvelle colonne 'Date_Parse'
        if "%d-%b-%Y" in list_formats:
            df['Datetime'] = pd.to_datetime(df[colomn_date], errors='coerce', format="%d-%b-%Y")
        else:
        ...
        df['Heure'].fillna(method="ffill", inplace=True)
...
def fcmb_bank_pdf_reader(pdf_path, password):
...
def fcmb_bank_classify_transaction(row, categories):
...
def fcmb_bank_transaction_labelisation(data, new_key_salary="", categories=LABELISATION):
    try:
        ...
        data['LABEL'] = data.apply(lambda row: fcmb_bank_classify_transaction(row, categories), axis=1)
        categories = del_employer_labelisation(categories)
        return data
    except Exception as e:
        print(f"[ERROR][utils.process_fcmb_bank.labelisation] {e}")
        data['LABEL'] = ""
        return data
...
def fcmb_bank_from_to(data, categories=LABELISATION, KEYWORDS=KEYWORDS):
    try:
        ...
        print(f"[ERROR][utils.process_fcmb_bank.fcmb_bank_from_to] {e}")
        data['FROM'] = ""
        data['TO'] = ""
        return data

"""
def fcmb_bank_parse_date(df, colomn_date, list_formats=[]):
...
def fcmb_bank_calcul_score(data):
...
if __name__ == '__main__':
    ...
    data = fcmb_bank_pdf_reader(pdf_path, password)    # pdf reader
    print(data.columns)
    data = standardize_column_name(data, COLUMNS_MAPPING) # rename column name as standard name

    data = fcmb_parse_date(data, "Date") # create new column Datetime from column Date
    ...
    data = fcmb_bank_from_to(data, categories=LABELISATION)

    print(data[["FROM", "TO"]])
    fcmb_bank_calcul_score(data)
